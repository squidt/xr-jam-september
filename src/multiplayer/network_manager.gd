...
func _on_lobby_joined(lobby: int, _permissions: int, _locked: bool, response: int) -> void:
	if response == 1:
		...
		match response:
			...
			5:
				FAIL_REASON = "Something unexpected happened!"
			6:
...
func _transition_to_lobby() -> void:
	...
	if MainStage.instance.lobby_2d:
		MainStage.instance.lobby_2d.goto_lobby()
...
func _on_join_lan() -> void:
	...
	var error = multiplayer_peer.create_client("localhost", 8563)
	if error != OK:
		display_error.emit("FAILED TO CREATE CLIENT\nCODE: " + str(error))
		multiplayer_peer.close()
		return
	print_debug("Valid lan join")
...
func _on_host_lan() -> void:
	lan = true
	multiplayer_peer = ENetMultiplayerPeer.new()
	var error = multiplayer_peer.create_server(8563, 3)  # allow 3 peers for 4 player lobby
	if error != OK:
